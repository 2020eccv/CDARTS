                ...
                for sub_epoch in range(sta_search_epoch, config.search_iter_epochs):
                    ...
                    for i in range(1):
                        if config.local_rank == 0:
                            ...
                            # genotype as a image
                            plot_path = os.path.join(config.plot_path, "Epoch_{:02d}".format(search_epoch+1))
                            caption = "Epoch_{}".format(search_epoch+1)
                            plot(genotype.normal, plot_path + "-normal", caption)
                            plot(genotype.reduce, plot_path + "-reduce", caption)
                            # save dict as json
                            genotype_file = os.path.join(config.plot_path, "genotypes_ep{:02d}.json".format(search_epoch+1))
                            genotypes_dict = {}
                            for idx, genotype in enumerate(genotypes):
                                genotypes_dict[str(idx)] = str(genotype)
                            js = json.dumps(genotypes_dict)
